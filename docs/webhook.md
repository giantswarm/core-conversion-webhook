...
Make sure you update the [webhook configuration](../helm/core-conversion-webhook/templates/webhook.yaml) to add the object which needs to be mutated or validated.
...
The URL path has to match with service path in the [webhook configuration](../helm/core-conversion-webhook/templates/webhook.yaml).
...
It's important to know `PatchOperation` only support `PatchAdd` or `PatchReplace`, see [patch.go](../core-conversion-webhook/pkg/admission/patch.go).
